...
export let nonPackOptions = [];
export let readyState = [];
export const MAX_READY_SLOTS = 5;

export function initGear(gearData) {
    allOptions = gearData;
    nonPackOptions = gearData.filter(g => g.category.toLowerCase() !== 'packs');
    readyState = Array(MAX_READY_SLOTS).fill(null).map(() => ({ gear: '', amt: 1, stowed: [] }));
    generateGearEntries();
    calculateLoad(); // Initial call
}
