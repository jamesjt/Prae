...
function populateRoleSelector() {
    ...
    // Filter and group ways by attack skill
    const strikeWays = waysData.filter(w => (w.props['attack skill'] || '').toLowerCase().includes('strike'));
    const blastWays = waysData.filter(w => (w.props['attack skill'] || '').toLowerCase().includes('blast'));
    const invokeWays = waysData.filter(w => (w.props['attack skill'] || '').toLowerCase().includes('invoke'));

    // Sort each group alphabetically by name
    strikeWays.sort((a, b) => a.name.localeCompare(b.name));
    blastWays.sort((a, b) => a.name.localeCompare(b.name));
    invokeWays.sort((a, b) => a.name.localeCompare(b.name));
    ...
    // Add groups in order: Strike, Blast, Invoke
    addGroup('Strike', strikeWays);
    addGroup('Blast', blastWays);
    addGroup('Invoke', invokeWays);
}
