...
function renderSidebar(data) {
  ...
  if (!sidebar) return console.error('Sidebar element missing');
...
function renderSections(data, term = '') {
  ...
  if (!Object.keys(filtered).length) {
    ...
    Object.keys(filtered).forEach(header => {
      ...
      const processed = lines.map(line => `<p class="indent-${getIndentLevel(line)}">${line}</p>`).join('');
      html += `
      ...
      filtered[header].subitems.forEach(sub => {
        ...
        const subProcessed = subLines.map(line => `<p class="indent-${getIndentLevel(line)}">${line}</p>`).join('');
        html += `
...
function filterData(data, term) {
  ...
  Object.keys(data).forEach(h => {
    ...
    const subMatches = data[h].subitems.filter(s => s.name.toLowerCase().includes(term) || s.details.toLowerCase().includes(term));
...
document.querySelectorAll('.nav-list a').forEach(link => {
  link.addEventListener('click', e => {
    ...
    if (target === 'character-creator') document.getElementById('creator-css').disabled = false;
  });
});
...
fetch(CSV_URL)
  ...
  .then(text => {
    const parsed = Papa.parse(text, { header: false, skipEmptyLines: true, dynamicTyping: false, delimitersToGuess: [','] });
    ...
    const dataRows = rows.slice(1).map(values => {
      ...
      return { Sections: values[sectionsIndex]?.trim() || '', Details: values[detailsIndex] || 'WIP' };
    }).filter(r => r.Sections);
