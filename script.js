...
function parseCSV(csvText) {
    ...
    try {
        ...
        if (sectionsIndex === -1) { console.warn('Sections header missing → using column 1'); sectionsIndex = 1; }
        let detailsIndex = headers.findIndex(h => h.toLowerCase().includes('detail') || h.toLowerCase() === 'c');
        if (detailsIndex === -1) { console.warn('Details header missing → using column 2'); detailsIndex = 2; }

        const dataRows = rows.slice(1).map((values, i) => {
    ...
    } catch (e) { console.error('CSV parse error:', e); throw e; }
}
...
// Render sections with dash indentation (dashes stay visible)
